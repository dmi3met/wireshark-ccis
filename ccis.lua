...
--ip_address = ProtoField.int32("ccis.ip_src", "from_ip", base.DEC)
...
--number_b_length = ProtoField.int32("ccis.number_b_length", "Number B length", base.DEC)
--number_b = ProtoField.int32("ccis.number_b", "Number B", base.DEC)
...
function ccis_protocol.dissector(buffer, pinfo, tree)
  ...
  --subtree:add_le("CCIS-MSU", buffer(0,8))
  ...
  if length>26 then
	 ...
	 -- subtree:add_le("UDP IP Remote: " .. udp_ip_remote)
	  ...
	  if opcode_name == "INVITE" then
		  ...
		  --number_b_length = string.format("%02X", num_b_hex)
